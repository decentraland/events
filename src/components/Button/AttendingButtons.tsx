...
export default function AttendingButtons(props: AttendingButtonsProps) {
  ...
  return <div className="AttendingButtons">
    ...
    {!fallbackShare && <>
      ...
      {(actions.provider || !isMobile) && <Button inverted primary size="small" className="share" disabled={loading || sharing || !approved} onClick={handleShare}>
        <img src={shareIcon} width="14" height="14" />
      </Button>}

      {!actions.provider && isMobile && <Button inverted primary size="small" className="share fluid" disabled={loading || sharing || !approved} onClick={handleShare}>
