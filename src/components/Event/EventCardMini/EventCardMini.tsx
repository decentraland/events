...
export default React.memo(function EventCardMini(props: EventCardMiniProps) {
  ...
  const handleJumpIn = useCallback((e: React.MouseEvent<any>) => e.stopPropagation(), [])
  ...
  return <Card
    ...
    onClick={handleClick}
  >
    {event && !props.loading && <JumpInPosition event={event} compact onClick={handleJumpIn} />}
    <div style={{ display: 'flex' }}>
      <div className="EventCardMini__Cover">
        <ImgFixed src={event?.image || ''} dimension="square" />
        {event && !props.loading && <div className="EventCardMini__Attendees">
