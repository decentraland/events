...
export const getCategoriesFetch = once(async () => await Events.get().getCategories())
...
export const getCategoriesOptionsActives = (categories: (EventCategoryAttributes)[]|null, userCategories: string[]): Option[] => {
  ...
  if (categories) {
    result = categories?.filter(
      ...
      (category) => ({ key: category.name, value: category.name, text: `page.events.categories.${category.name}` })
    )
  }
