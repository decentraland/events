...
describe("when publishing messages with SNS", () => {
    ...
    beforeEach(() => {
      ...
      clientSend
        .mockResolvedValueOnce({
          Successful: events
            .slice(0, 10)
            .map((_, i) => ({ MessageId: `m-${i}` })),
        })
        .mockResolvedValueOnce({
          Successful: events
            .slice(10, 20)
            .map((_, i) => ({ MessageId: `m-${10 + i}` })),
        })
        .mockResolvedValueOnce({
          Successful: events
            .slice(20, 25)
            .map((_, i) => ({ MessageId: `m-${20 + i}` })),
        })
    })
  ...
  describe("and attributes are attached to messages", () => {
    let events: any[]
    let capturedCommandArgs: any

    beforeEach(() => {
      events = createEvents(1)
      ;(PublishBatchCommand as unknown as jest.Mock).mockImplementation(
        (args) => {
    ...
    it("should set type and subType as SNS attributes", async () => {
      ...
      expect(res.failedEvents).toHaveLength(0)
      expect(
        capturedCommandArgs.PublishBatchRequestEntries[0].MessageAttributes.type
          .StringValue
      ).toBe("event")
      expect(
        capturedCommandArgs.PublishBatchRequestEntries[0].MessageAttributes
          .subType.StringValue
      ).toBe("sub")
    })
  })
